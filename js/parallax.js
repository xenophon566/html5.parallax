...
jQuery.fn.parallScroll = function (settings, selector) {
    settings = jQuery.extend({
        ...
        hideType: function ($elem) {
            $elem.hide()
        }, //Customise hidden
        showType: function ($elem) {
            $elem.show()
        }, //Customise shown
    ...
    var parallax = new function () {
        ...
        settings.scrollLimit *= bRatio;
        settings.scrollRatio *= bRatio;
        settings.naviRatio *= bRatio;
        var bound = settings.scrollAxis == 'x' ? win.width() : win.height();

        //initiate parallax effects starting
        this.init = function () {
            parallax.parallax.init_parallax()
        }
        ...
        this.parallax = new function () {
            ...
            this.init_parallax = function () {
                ...
                $(document).on("keyup keypress keydown", elem, function (event) {
                    ...
                    if (event.keyCode == 37 || event.keyCode == 38) var delta = 1;
                    if (event.keyCode == 39 || event.keyCode == 40) var delta = -1;
                    var keyCodeArr = [37, 38, 39, 40];
                    if (!!~keyCodeArr.indexOf(event.keyCode)) {
                ...
                _this.scrollEasing = function (useEasing) {
                    if (useEasing) {
                        ...
                        if (settings.scrollAxis == 'x') {
                            docuElem.stop(true, false).animate({
                                scrollLeft: _this.scrollPos
                            }, {
                                duration: settings.scrollRatio,
                                step: function () {
                                    var tmpScroll = docuElem.scrollLeft() ? docuElem.scrollLeft() : elem.scrollLeft();
                                    if (tmpScroll != scrollTmp) {
                                        _this.scrollUpdate(tmpScroll);
                                        scrollTmp = tmpScroll;
                                    }
                                }
                            });
            ...
            this.aniHandler = function (scrollPx) {
                ...
                elem.find('[data-rotate]').each(function () {
                    var optRotate = $(this).data("rotate"),
                        initRotate = $(this).data("init-rotate"),
                        speed = +optRotate.speed || 5,
                        direct = +optRotate.direct || 0,
                ...
                elem.find('[data-scale]').each(function () {
                    var optScale = $(this).data("scale"),
                        initScale = $(this).data("init-scale"),
                        speed = +optScale.speed * bRatio || 100,
                        resize = +optScale.resize || 1,
                        direct = +optScale.direct || 0,
                        max = +optScale.max || 5,
                        min = +optScale.min || 1,
                        sdist = +optScale.sdist * bRatio || 0,
                        edist = +optScale.edist * bRatio || bound,
                        dist = +initScale.dist * bRatio || 0,
                        scale = +initScale.scale || 1;
                ...
                elem.find('[data-frame]').each(function () {
                    var optFrame = $(this).data("frame"),
                        initFrame = $(this).data("init-frame"),
                        speed = +optFrame.speed * bRatio || 100,
                        sdist = +optFrame.sdist * bRatio || 0,
                        edist = +optFrame.edist * bRatio || bound,
                        dist = +initFrame.dist * bRatio || 0,
                        fNum = +$(this).nextAll().length,
                        fNow = +initFrame.now || 0,
                        fNext = +initFrame.next || 0;
                    ...
                    if (fNext != fNow) {
                        ...
                        $(this).nextAll().each(function (i) {
                            if (i == fNext) $(this).css('display', 'block');
                        });
                ...
                elem.find('[data-mask]').each(function () {
                    var optMask = $(this).data("mask"),
                        initMask = $(this).data("init-mask"),
                        speed = +optMask.speed * bRatio || 100,
                        clip = +optMask.clip || 10,
                        direct = +optMask.direct || 0,
                        axis = +optMask.axis || 0,
                        sdist = +optMask.sdist * bRatio || 0,
                        edist = +optMask.edist * bRatio || bound,
                        dist = +initMask.dist * bRatio || 0,
                        myMask = $(this).next().length ? $(this).next() : $(this),
                        maskW = +myMask.width(),
                        limitW = +$(this).parent().width(),
                        maskH = +myMask.height(),
                        limitH = +$(this).parent().height();

                    if (scrollDirect - scrollPx > 0) {
                        if (scrollPx > sdist && scrollPx < sdist + edist && scrollPx > dist) {
                            if (!axis) maskW = direct ? maskW - clip : maskW + clip;
                            else maskH = direct ? maskH + clip : maskH - clip;
                            if (maskW > limitW) maskW = limitW;
                            if (maskW < 0) maskW = 0;
                            if (maskH > limitH) maskH = limitH;
                            if (maskH < 0) maskH = 0;
                            dist += speed;
                        }
                    } else if (scrollDirect - scrollPx < 0) {
                ...
                elem.find('[data-sprite]').each(function () {
                    var optSprite = $(this).data("sprite"),
                        ...
                        direct = +optSprite.direct || 0,
                        axis = +optSprite.axis || 0,
                        posArr = $(this).css("background-position").split(' '),
                        spriteX = +posArr[0].replace(/[^0-9-]/g, ''),
                        spriteY = +posArr[1].replace(/[^0-9-]/g, '');
                ...
                elem.find('[data-move]').each(function () {
                    var optMove = $(this).data("move"),
                        initMove = $(this).data("init-move"),
                        speed = +optMove.speed * bRatio || 100,
                        moving = +optMove.moving * bRatio || 10,
                        direct = +optMove.direct || 0,
                        max = +optMove.max * bRatio || 100,
                        min = +optMove.min * bRatio || 0,
                        sdist = +optMove.sdist * bRatio || 0,
                        edist = +optMove.edist * bRatio || bound,
                        dist = +initMove.dist * bRatio || 0,
                        move = +initMove.posi * bRatio || 0;

                    if (scrollDirect - scrollPx > 0) {
                        if (scrollPx > sdist && scrollPx < sdist + edist && scrollPx > dist) {
                            move = direct ? move + moving : move - moving;
                ...
                elem.find('[data-animated]').each(function () {
                    var optAnimated = $(this).data("animated"),
                        sdist = +optAnimated.sdist * bRatio || 0,
                        edist = +optAnimated.edist * bRatio || bound,
            ...
            this.naviScroll = function (scrollPx) {
                ...
                navi.find("a.active").each(function () {
                ...
                for (var naviId in this.naviLoc) {
                    temp[i] = {};
                    temp[i].myNaviId = naviId;
                    temp[i].myNaviLoc = this.naviLoc[naviId] * bRatio;
                    i++;
                }
                temp = temp.sort(function (a, b) {
                    return a.myNaviLoc < b.myNaviLoc ? 1 : -1;
                });
            ...
            this.naviClick = function (naviId) {
                ...
                if (settings.scrollAxis == 'x') {
                    docuElem.stop(true, true).animate({
                        scrollLeft: this.naviLoc[naviId] * bRatio
                    }, {
                        duration: settings.naviRatio,
                        step: naviUpdate,
                        complete: naviUpdate
                    });
                } else {
                    docuElem.stop(true, true).animate({
                        scrollTop: this.naviLoc[naviId] * bRatio
                    }, {
                        duration: settings.naviRatio,
                        step: naviUpdate,
                        complete: naviUpdate
